...
public class TestSolrJ {
	...
	public void testSolrJAddDocument() throws Exception {
        ...
        SolrServer solrServer = new HttpSolrServer("http://192.168.244.146:8080/solr");
        ...
        document.addField("item_sell_point", "送鼠标一个，耳机一个哟！");
        document.addField("item_price", 20000);
        document.addField("item_image", "http://www.123.ipg");
        document.addField("item_category_name", "电器");
        document.addField("item_desc", "这是一款最新的电脑，质量好，值得您信赖！！");
        // 把文档写入索引库
        solrServer.add(document);
	...
	@Test
	public void testSolrDeleteDocument() throws Exception {
	    // 创建一个SolrServer对象，即HttpSolrServer对象，需要指定solr服务的url
	    SolrServer solrServer = new HttpSolrServer("http://192.168.244.146:8080/solr");
	    // 通过id来删除文档
